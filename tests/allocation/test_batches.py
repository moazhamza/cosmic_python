...
def test_allocating_to_a_batch_reduces_the_available_quantity():
    batch = Batch(reference="batch-001", sku="SMALL-TABLE", purchased_quantity=20, eta=date.today())
...
def make_batch_and_line(sku, batch_qty, line_qty):
    return (
        Batch(reference="batch-001", sku=sku, purchased_quantity=batch_qty, eta=date.today()),
...
def test_cannot_allocate_if_skus_do_not_match():
    batch = Batch(
        reference="batch-001", sku="UNCOMFORTABLE-CHAIR", purchased_quantity=100, eta=None
    )
